...
<template>
    <li class="flex items-center justify-between rounded-sm hover:bg-[#2A2929]" @mouseenter="isHover = true"
        @mouseleave="isHover = false">
        <div class="flex items-center w-full py-1.5">
            <div v-if="isHover" class="w-[40px] ml-[14px] mr-[6px] cursor-pointer">
                <Play v-if="!isPlaying" fillColor="#FFF" :size="25" @click="useSong.playOrPauseThisSong(artist, track)" />
                <Play v-else-if="isPlaying && currentTrack.name !== track.name" fillColor="#FFF" :size="25"
                    @click="useSong.loadSong(artist, track)" />
                <Pause v-else fillColor="#FFF" :size="25" @click="useSong.playOrPauseSong()" />
            </div>
